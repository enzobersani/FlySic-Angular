export class RangeDate {
  ...
  static createCompleteInstace(
    initialDate: Date | null = null,
    finalDate: Date | null = null
  ): RangeDate {
    const initial = initialDate ?? new Date(1, 1, 1);
    const final = finalDate ?? new Date(9999, 31, 12);

    var rangeDate = new RangeDate();
    rangeDate.initialDate = new Date(initial.getFullYear(), initial.getMonth(), 1);
    rangeDate.finalDate = new Date(final.getFullYear(), final.getMonth() + 1 , 0);
  ...
  toString() {
    return (
      this.initialDate?.toLocaleDateString() +
      ' - ' +
      this.finalDate?.toLocaleDateString()
    );
  }
}
